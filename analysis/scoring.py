...
def calculate_aggregate_score(company: Company) -> CompanyScore:
    ...
    data_points = 0
    total_data_points = 6
    if hasattr(fin, "q_sales") and fin.q_sales is not None:
        data_points += 1
    if fin and fin.debt_to_equity is not None:
        data_points += 1
    if fin and fin.qoq_sales_growth is not None:
        data_points += 1
    ...
    # Growth Momentum
    if (
        fin.qoq_sales_growth
        and fin.qoq_sales_growth >= SCORING_CONFIG.min_qoq_sales_growth
    ):
        score += 1
    ...
    if (
        fin.opm_expansion
        and fin.opm_expansion >= SCORING_CONFIG.min_operating_margin_expansion
    ):
        score += 1
    ...
    if val and val.is_bargain:
        ...
        reasons.append(
            f"Undervalued vs Graham Intrinsic Value ({val.graham_intrinsic_value})"
        )
    elif (
        val
        and val.graham_intrinsic_value
        and company.price
        and company.price > val.graham_intrinsic_value * 1.5
    ):
        risks.append(
            f"Overvalued vs Graham Intrinsic Value ({val.graham_intrinsic_value})"
        )
    ...
    if risks:
        recommendations.append(
            "Ensure risks are hedged or sizing is adjusted appropriately."
        )
