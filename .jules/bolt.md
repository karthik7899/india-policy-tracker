...
## 2026-08-20 - Sequential IO inside async loops
**Learning:** Sequential `await` calls inside a `for` loop in async functions (like `scrape_pib_pli_approvals_async`) block the event loop, causing the application to process network tasks serially.
**Action:** Use `asyncio.gather` with a list of coroutines to process the loop tasks concurrently, preventing event-loop blocking and significantly reducing network I/O wait time.
