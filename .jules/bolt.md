...
## 2026-06-25 - Watchlist Flattening Optimization in Exchange Events
**Learning:** In `providers/exchange_events.py`, the `match_watchlist_company` function was being called in a loop for every announcement or deal row, repeatedly iterating over the nested watchlist structure and calling `_normalise` on holding names. This is an O(N^2) operation involving redundant string normalizations.
**Action:** Pre-compute a `flat_watchlist` (a list of `(_normalise(name), ticker)` tuples) outside the loop and pass it into the matching function. This avoids repetitive string normalizations and converts the inner loop into a simpler linear scan of pre-normalized tuples, significantly reducing the overhead for large payloads.
