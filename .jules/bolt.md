...
## 2024-06-25 - O(1) Watchlist Lookups in Curation
**Learning:** Found an O(N^2) bottleneck when verifying if a newly discovered emerging candidate was already in the watchlist. The inner loop was scanning the dictionary-of-lists `watchlist` structure via `any()` on every evaluation.
**Action:** Replaced the O(N^2) nested loop with an O(1) hash map lookup by pre-flattening the watchlist tickers into a set `watchlisted_tickers` before the candidate evaluation loop, updating the set synchronously when additions or rotations happen.
