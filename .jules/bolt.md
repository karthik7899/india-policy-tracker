...
## 2026-07-30 - [BeautifulSoup C-Based Parsing Optimization]
**Learning:** For DOM parsing tasks that require full parsing rather than just tag stripping (e.g., scraping tables from Screener.in), the built-in `html.parser` is significantly slower than the C-based `lxml` parser.
**Action:** Explicitly pass the 'lxml' parser instead of Python's built-in 'html.parser' (e.g., `BeautifulSoup(html, "lxml")`). It yields a 30-50% parsing speedup for large DOM processing.
