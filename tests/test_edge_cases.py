...
def test_missing_yahoo_fields(monkeypatch):
    class MockHist:
        ...
        def __getitem__(self, key):
            class Series:
                @property
                def iloc(self): return [-1, 100.0]
            return Series()
            
    class MockTicker:
        def history(self, *args, **kwargs): return MockHist()
        @property
        def info(self): return {"shortName": "Test Corp"} # Missing everything else
        
    monkeypatch.setattr("providers.yahoo.get_cached_ticker", lambda *args, **kwargs: MockTicker())
...
def test_invalid_stock_entries(monkeypatch):
    ...
    monkeypatch.setattr("analysis.rotation.resolve_ticker_from_name", lambda *args, **kwargs: (None, None))
    
    # brief_data represents news items, so it must have 'title' and 'summary'
    brief_data = {sector: [{"title": "Invalid Corp Limited announces things", "summary": "Stuff happens", "published": "Sat, 27 Jun 2026 10:00:00 GMT"}]} 
